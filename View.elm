...
memoHelper : Model -> a -> b -> Html Msg
memoHelper model _ _ =
    div
        [ style
            [ ( "height", toPx ((model.calendar.numWeeks + 2) * he) )
            , ( "position", "absolute" )
...
composeMonths model =
    let
        ( s, e ) =
            weekStartRange (model.calendar.numWeeks + 2)
                (model.scrolls.months.shift)
                model.calendar.focused

        months =
            monthsArray s e empty
            ...
            List.foldl
                (\a b ->
                    let
                        ...
                        recentmonth =
                            x_get (Array.length b) dda months
                    in
                        if
                            sameMonth recentmonth checkweek
                                && Array.length b
                                < Array.length months
                        then
                            push a b
                ...
                empty
                (List.range 0 <| model.calendar.numWeeks + 2)
        ...
        scrolled =
            model.offsets.months.y

        ( zero, one ) =
            ( x_get 0 (withDefault 0) prelocations
        ...
        emp =
            List.repeat (Array.length months) []

        ( locations, adjust ) =
            if gap * he < diff * he + scrolled then
...
composeWkdays model =
    let
        wid =
            100 / (toFloat <| Array.length model.calendar.wkdays)
    in
        Array.toList <|
            Array.map
                (\a ->
...
composeDays model =
    let
        ...
        combined =
            initialize (Array.length model.memoized.days) identity
    in
        Array.toList <|
            Array.map
                (\a ->
                    let
                        bord =
                            withDefault 0 <| get a model.memoized.borders
